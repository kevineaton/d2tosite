...
func ParseD2(input []byte, options *ParseOptions) ([]byte, error) {
	...
	renderOptions := &d2svg.RenderOpts{}

	diagram, _, err := d2lib.Compile(context.Background(), string(input), compileOptions, renderOptions)
	if err != nil {
		return bytes, err
	}

	out, err := d2svg.Render(diagram, renderOptions)
